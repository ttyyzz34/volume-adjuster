...
void MX_FREERTOS_Init(void) {
	...
	KeyQueueHandle = osMessageQueueNew(1, sizeof(uint16_t), &KeyQueue_attributes);
...
void Key_Task(void *argument)
{
	...
	for(;;)
	{
		if(osMessageQueueGet(KeyQueueHandle, &key, 0, osWaitForever) == osOK)
		{
			...
			if(key == 1)
			{
				while(HAL_GPIO_ReadPin(KEY_A_GPIO_Port, KEY_A_Pin) == GPIO_PIN_SET)
				{
					osDelay(1);
				}
			}
			...
			{
				while(HAL_GPIO_ReadPin(KEY_B_GPIO_Port, KEY_B_Pin) == GPIO_PIN_SET)
				{
					osDelay(1);
				}
			}
			...
			{
				while(HAL_GPIO_ReadPin(KEY_B_GPIO_Port, KEY_Pin) == GPIO_PIN_SET)
				{
					osDelay(1);
				}
			}
